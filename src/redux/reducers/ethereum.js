import {
    LOAD_WEB3_SUCCESS,
    LOAD_WEB3_FAIL,
    LOAD_BLOCKCHAIN_DATA_SUCCESS,
    LOAD_BLOCKCHAIN_DATA_FAIL,
    LOAD_ERTEX_CONTRACT_SUCCESS,
    LOAD_ERTEX_CONTRACT_FAIL,
    LOAD_NFT_TOTALSUPPLY_SUCCESS,
    LOAD_NFT_TOTALSUPPLY_FAIL,
    LOAD_ERTEX_NFTS_SUCCESS,
    LOAD_ERTEX_NFTS_FAIL

} from '../actions/types';

const initialState = {
    web3: null,
    blockchain: null,
    ertexAddress: null, // Cambiamos nombre para ser más claros
    ertexLoaded: false,
    totalSupply: null,
    nfts: []
};

export default function ethereum(state = initialState, action) {
    const { type, payload } = action;

    switch (type) {
        case LOAD_ERTEX_NFTS_SUCCESS:
            return {
                ...state,
                nfts: payload
            };
        case LOAD_NFT_TOTALSUPPLY_SUCCESS:
            return {
                ...state,
                totalSupply: payload
            };
        case LOAD_ERTEX_CONTRACT_SUCCESS:
            return {
                ...state,
                ertexAddress: payload, // Guardamos el string '0x123...'
                ertexLoaded: true
            };
        case LOAD_WEB3_SUCCESS:
            return {
                ...state,
                web3: true // Solo marcamos que web3 cargó, no guardamos el objeto
            };
        case LOAD_BLOCKCHAIN_DATA_SUCCESS:
            return {
                ...state,
                blockchain: payload
            };
        case LOAD_ERTEX_NFTS_FAIL:
            return {
                ...state,
                nfts: []
            };
        case LOAD_NFT_TOTALSUPPLY_FAIL:
            return {
                ...state,
                totalSupply: null
            };
        case LOAD_ERTEX_CONTRACT_FAIL:
            return {
                ...state,
                ertexAddress: null, // Cambiado de 'ertex' a 'ertexAddress'
                ertexLoaded: false  // También es buena idea resetear el booleano
            };
        case LOAD_BLOCKCHAIN_DATA_FAIL:
            return {
                ...state,
                blockchain: null
            };
        case LOAD_WEB3_FAIL:
            return {
                ...state,
                web3: null
            };
        default:
            return state;
    }
}